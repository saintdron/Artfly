...
function smarty_function_html_doctype($params, &$smarty)
{
  ...
  if ($xhtml) {
    ...
    if (stristr($_SERVER['HTTP_ACCEPT'],"application/xhtml+xml")) {
      if ($sendheaders) header("Content-Type: application/xhtml+xml; charset=$encoding");
      $GLOBALS["HTML_DOCTYPE"] = "xml";//******
    }
    elseif (stristr($_SERVER['HTTP_ACCEPT'],"application/xml")) {
      if ($sendheaders) header("Content-Type: application/xml; charset=$encoding");
      $GLOBALS["HTML_DOCTYPE"] = "xml";//******
    }
    elseif (stristr($_SERVER['HTTP_ACCEPT'],"text/xml")) {
      if ($sendheaders) header("Content-Type: text/xml; charset=$encoding");
      $GLOBALS["HTML_DOCTYPE"] = "xml";//******
    }
    ...
    //Send everyone else text/html
      ...
      if ($sendheaders) header("Content-Type: text/html; charset=$encoding");
