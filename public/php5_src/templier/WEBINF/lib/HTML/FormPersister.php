...
class HTML_FormPersister extends HTML_SemiParser 
{
    ...
    function tag_input($attr)
    {
        ...
        if (isset($attr['label'])) {
            ...
            unSet($attr['label']);
            $attr[$right? '_right' : '_left'] = '<label for="'.htmlspecialchars($attr['id']).'">' . $text . '</label>';
        }
        ...
        foreach ($options as $k=>$text) {
            if (is_array($text)) {
                ...
                foreach ($text as $ko=>$v) {
                    $opt = array('_tagName'=>'option', 'value'=>$ko, '_text'=>htmlspecialchars(strval($v)));
                ...
                // single option
                $opt = array('_tagName'=>'option', 'value'=>$k, '_text'=>htmlspecialchars($text));
